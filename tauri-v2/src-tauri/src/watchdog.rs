...
pub async fn check_and_restrain(processes: &[ProcessInfo]) {
    ...
    let total_cpu_percent = if logical_cores > 0.0 { total_cpu_sum / logical_cores } else { 0.0 };
...
async fn restrain_processes(processes: &[ProcessInfo], excludes: &[String]) {
    ...
    for p in processes {
        ...
        let is_target_pri = current_pri == "Normal" || current_pri == "AboveNormal" || current_pri == "High"; // Include High? Maybe.
        
        if !is_target_pri { continue; }
        
        if p.pid == foreground_pid { continue; }
        
        if excludes.iter().any(|ex| p.name.to_lowercase().contains(&ex.to_lowercase())) {
            continue;
        }
        ...
        if p.cpu_usage < 1.0 { continue; }

        // ACT: Restrain
        if !restrained.contains(&p.pid) {
            tracing::info!("ProBalance: Restraining PID {} ({}) - CPU: {}", p.pid, p.name, p.cpu_usage);
